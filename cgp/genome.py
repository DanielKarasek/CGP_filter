...
class Genome:
    ...
    def __init__(
        ...
        self.is_active_nodes_valid = False
        self.active_nodes = [] # stores active nodes of the genome
    ...
    # xkaras38
    def distance(self, other: "Genome") -> int:
        """Calculate the distance between the input and output nodes.
        ...
        """
        dict_of_active = cgp.CartesianGraph(self)._determine_active_nodes()
        # popping input and output columns
        dict_of_active.pop(self._n_columns)
        # change dict into list of active nodes
        list_of_active = [item for subset in dict_of_active.values() for item in subset]

        primitives_dict = {primitive.__name__: 0 for primitive in self.primitives}

        for gen1, gen2 in zip(self.dna[skip_triplets_start:-skip_triplets_end],
                              other.dna[skip_triplets_start:-skip_triplets_end]):
            if gen1 != gen2:
                distance += 1
        return distance
    ...
    # xkaras38
    def calculate_count_per_function(self) -> Dict[str, int]:
        """
        ...
        for node in list_of_active:
            primitives_dict[node.__class__.__name__] += 1
    ...
    def clone(self) -> "Genome":
        ...
        new.is_active_nodes_valid = self.is_active_nodes_valid
        new.active_nodes = self.active_nodes.copy()
