...
def detection_image_processing(target_image: np.ndarray):
...
def create_regression_dataset(path_to_image: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
  dataset_x, dataset_y, noised_downscaled = create_dataset_from_image(path_to_image)
  filter_vector = np.where(dataset_x[:, 3, 3] == dataset_y, 0, 1).astype(bool)
